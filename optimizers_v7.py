...
class _APGDBase:
    ...
    def __init__(self, model, base_lr: float = 0.01,
        ...
        # Phase candidates strictly inside (−π/2, π/2), θ=0 at centre
        raw = torch.linspace(-np.pi / 2 + 0.1, np.pi / 2 - 0.1, K).tolist()
        raw[K // 2] = 0.0
    ...
    def _debug_print(self, step, f0, g_raw, g_clipped,
        ...
        if regime == 'saddle' and phase_losses:
            entries = "  ".join(
    ...
    def step(self, inputs, labels, criterion) -> tuple:
        ...
        if bt0_iters >= self.saddle_bt_thresh:
            ...
            if theta != 0.0:
                rho, bt1_iters = self._backtrack(
                    inputs, labels, criterion, params, p_grads, theta, f0)
                regime = 'saddle'
            else:
